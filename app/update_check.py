...
Purely informational — nothing is ever installed automatically; updating is
...
log = logging.getLogger("mediapulse.update")
        ...
        async with httpx.AsyncClient() as client:
            r = await client.get(
                ...
                headers={"Accept": "application/vnd.github+json",
                         "User-Agent": "mediapulse-update-check"},
                timeout=20.0,
            )
...
async def run() -> None:
    if not GIT_SHA:
        log.info("no GIT_SHA baked into this build — update checks disabled")
        return
    while True:
        try:
