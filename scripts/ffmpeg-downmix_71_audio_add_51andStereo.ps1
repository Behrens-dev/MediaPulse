...
foreach ($file in $mediaFiles) {
    ...
    ffmpeg -i "$inputPath" -map 0:v -c:v copy -map 0:a:0 -c:a:0 copy -map 0:a:0 -c:a:1 aac -filter:a:1 "pan=5.1|FL=FL|FR=FR|FC=FC|LFE=LFE|BL=BL|BR=BR" -b:a:1 384k -map 0:a:0 -c:a:2 aac -filter:a:2 "pan=stereo|FL=FL+FC+BL+SL|FR=FR+FC+BR+SR" -b:a:2 192k -map 0:s? -c:s copy "$outputPath"
